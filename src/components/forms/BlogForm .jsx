...
const BlogForm = ({ isUpdate, updateTitle, updateSlug, updateCategory, updateAuthor, updateMetaTitle, updateMetaDescription, updateTags, updateThumbnail, itemId, updateAltText, updateContent }) => {
  ...
  const { title, slug, author, altText, category, metaDescription, metaTitle, tags } = getValues()
    ...
    if (!isUpdate && !updatedAsDraft) {
      try {
        await uploadThumbnail(thumbnailFormData, onPage).then(async (res) => {
          ...
          const randomId = `${Math.random().toString(36).substr(2, 9)}${Math.random().toString(32).substr(2, 9)}`
          setDraftId(randomId.toString())
          await updateBlog({
            title, slug, itemId: randomId, author,
  ...
  const nextStep = async () => {
    setIsFormValid(Object.keys(formState.touchedFields).length === 0)
    if (currentStep.step === steps.length - 1) return;
    if (isUpdate && isFormValid) setCurrentStep(steps[currentStep.step + 1])
  ...
  const PublishBlog = async () => {
    await uploadThumbnailToServer().then(async (res) => {
      const result = await updateBlog({
        title, slug, itemId: isUpdate ? itemId : draftId, isActive: true,
  ...
  return (
    <section className="w-full h-full py-4  rounded-lg">
      <div className={'px-6 flex flex-col  h-full  items-center   mx-auto  gap-2 ' + `${currentStep.step === 1 ? "w-full" : width}`}>
        ...
        {
          currentStep.step === 2 &&
          <Preview NameOrTitle={title} PublishFunc={PublishBlog} altText={altText} message={message} saveAsDraftFunc={saveAsDraft} success={success} author={author} slug={slug} category={category} thumbnailUrl={thumbnailUrl} successDisplay={successDisplay} />
        }
